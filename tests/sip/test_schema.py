...
@pytest.mark.parametrize(
    "field", ["bagpath", "dsid", "mimetype", "creator", "size", "checksums"]
)
...
def test_validate_min_ds_field_sizes(field, min_size, sip_json):
    ...
    Fields with minimum size constraints should raise ValidationError 
    if too small.
    """
    sip_json["contentFiles"][0][field] = "x" * (
        min_size - 1
    )  # Set field to a string that is too short
    with pytest.raises(jsonschema.ValidationError, match=field):
        jsonschema.validate(instance=sip_json, schema=gams_schema)
...
def test_validate_extra_field(sip_json):
    ...
    with pytest.raises(jsonschema.ValidationError, match="extraField"):
        jsonschema.validate(instance=sip_json, schema=gams_schema)
...
def test_invalid_checkum_prefix(sip_json):
    ...
    sip_json["contentFiles"][0]["checksums"][0] = sip_json["contentFiles"][0][
        "checksums"
    ][0].replace("md5", "invalidprefix")
...
def test_invalid_checksum_value(sip_json):
    ...
    sip_json["contentFiles"][0]["checksums"][0] = (
        "md5 " + "g" * 32
