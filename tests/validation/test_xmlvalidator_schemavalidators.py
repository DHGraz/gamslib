...
def test_schematron_validator_init(lxml_schematron_validator, saxon_schematron_validator):
    "Test if the validator used in the validator factory is the correct one."
    assert isinstance(lxml_schematron_validator.schema_validator, lxml.isoschematron.Schematron)
    assert isinstance(saxon_schematron_validator.schema_validator, saxonche.PyXsltExecutable)   # pylint: disable=c-extension-no-member
...
def test_schematron_lxml_validator_invalid_document(lxml_schematron_validator, lazy_shared_datadir):
